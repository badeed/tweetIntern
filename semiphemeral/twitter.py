...
class Twitter(object):
    ...
    def import_tweet_and_thread(self, tweet):
        ...
        if tweet.in_reply_to_status_id:
            ...
            parent_tweet = (
                self.common.session.query(Tweet)
                .filter_by(status_id=tweet.in_reply_to_status_id)
                .first()
            )
            if not parent_tweet:
                # If not, import it
    ...
    def calculate_excluded_threads(self):
        ...
        if self.common.settings.get("tweets_threads_threshold"):
            threads = (
                ...
                .filter(
                    Tweet.retweet_count
                    >= self.common.settings.get("tweets_retweet_threshold")
                )
                .filter(
                    Tweet.favorite_count
                    >= self.common.settings.get("tweets_like_threshold")
                )
                .all()
            )
    ...
    def unlike(self, filename):
        ...
        with open(filename, encoding="UTF-8") as f:
            ...
            for obj in likes:
                ...
                if "like" not in obj:
                    click.echo(
                        "JSON object expected to be a list of dicts that contain 'like' fields"
                    )
                    return
                like = obj["like"]
                if "tweetId" not in like:
                    click.echo(
                        "JSON object expected to be a list of dicts that contain 'like' fields that contain 'tweetID' fields"
                    )
                    return
        ...
        click.secho(
            "Making a list of liked tweet status_ids".format(len(likes)), fg="cyan"
        )
        like_status_ids = []
        for obj in likes:
        ...
        click.secho(
            "Like history has {} tweets".format(len(like_status_ids)), fg="cyan"
        )
        ...
        if len(remaining_status_ids) > 0:
            click.secho(
                "Fetching remaining {} tweets from API".format(
                    len(remaining_status_ids)
                ),
                fg="cyan",
            )
            count = 0
    ...
    def relike_unlike_tweets(self, datetime_threshold, tweets):
        ...
        click.secho(
            "Re-liking and unliking {} liked tweets".format(len(tweets)), fg="cyan"
        )
        count = 0
        for tweet in tweets:
            if tweet.created_at < datetime_threshold and not tweet.is_unliked:
                if tweet.favorited:
                    ...
                    try:
                        ...
                        if "status code = 429" in str(e) and e.api_code == None:
                            ...
                            click.secho(
                                "You can only like 1000 tweets per 24 hours and you have hit the limit",
                                bold=True,
                            )
                            click.echo(
        ...
        with open(filename, encoding="UTF-8") as f:
            ...
            for obj in conversations:
                ...
                if "dmConversation" not in obj:
                    click.echo(
                        "JSON object expected to be a list of dicts that contain 'dmConversation' fields"
                    )
                    return
                dm_conversation = obj["dmConversation"]
                if "messages" not in dm_conversation:
                    click.echo(
                        "JSON object expected to be a list of dicts that contain 'dmConversations' fields that contain 'messages' fields"
                    )
                    return
            ...
            for message in obj["dmConversation"]["messages"]:
                ...
                if created_timestamp <= datetime_threshold:
                    ...
                    try:
                        ...
                        click.echo(
                            "Deleted DM {}, id {}".format(
                                created_timestamp.strftime("%Y-%m-%d"), dm_id
                            )
                        )
                        count += 1
                    except tweepy.error.TweepError as e:
